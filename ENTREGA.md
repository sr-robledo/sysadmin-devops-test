...
- **systemd de WSL2 funciona** (con `systemd=true` en
  ...
  correctamente.
- **La distro ya tiene un usuario con `sudo` (`sacerdote`)**.
  El script de aprovisionamiento crea además un usuario
  `sysadmin` con sudo NOPASSWD para la administración habitual.
